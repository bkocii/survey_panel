...
(function (w, d) {
    ...
    return `
      <div class="preview-card" data-qid="${p.id || ''}">
        <div class="card-head">
          ...
          <div class="meta">
            <button type="button" class="btn btn-sm btn-outline-secondary" data-action="edit" data-qid="${p.id || ''}" disabled>
              Edit (coming soon)
            </button>
          </div>
        </div>
  ...
  async function init() {
    ...
    // Also listen to generic input events to be safe
    d.addEventListener('input', (e) => {
      if ((e.target && (e.target.closest('#question-form') || e.target.id?.startsWith('id_matrix_') || e.target.id?.startsWith('id_choices-')))) {
        updatePreview();
      }
    });
